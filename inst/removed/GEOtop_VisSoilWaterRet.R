...
#' 
#' 		
#' @param alpha	alpha van Genuchten parameter [1/m]; inverse of a length - scale parameter
			
#' @param n	 N van Genuchten parameter [-]; shape parameter
		
#' @param theta_sat	saturated soil water content [m³/m³]
			
#' @param theta_res				residual soil water content [m³/m³]
			
#' @param theta_pwp				permanent wilting point [m³/m³]
		
#' @param thata_fc 				field capacity [m³/m³]
			
#' @param	observed			numeric matrix, providing observed soil moisture data [m³/m³] (first column) and water potential [hPa] (second column)
			
#' @param  add_ref_curves				boolean, default = TRUE: reference soil water retention curves are added for clay, loam and sand; van Genuchten parameterisation according to ROSETTA manual
			
#' @param pdf				boolean, default = FALSE: visualisation is saved in pdf if TRUE
			
#' @param png					boolean, default = TRUE: visualisation is saved in png if TRUE; if png=FALSE and pdf=FALSE vis is ploted in x11()
		
#' @param accurate  default = 10: water potential is given as \emph{seq(1,10^7,10)}, for accurate plotting set argument 1
			
#' @param ksat		saturated hydraulic conductivity [mm/s]
			
#' @param colors		color spezification for water retention curves, default = NULL: grey color range is used
			
#' @param plot_khy boolean, default = TRUE: plot Mualem model - relation between hydraulic conducivity and water potential
#' 
#' 
#' @export 
#' 
...
#' 
#' 
#' @details If multiple vanGenuchten - Mualem models sould be visualised, simply provide alpha, n, theta_sat, theta_res, and ksat values as vectors. Provide alpha as a named vector in order to specify legend names. 
#' 
#' 
